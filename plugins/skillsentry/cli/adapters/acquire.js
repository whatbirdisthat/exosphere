...
export async function resolveInput(target) {
    ...
    throw new AuditError('UNRESOLVABLE_INPUT', `Cannot resolve input: ${target} is neither an existing directory nor a recognised git URL.`);
}
/** Acquire the audited source read-only. git-url → hostile shallow clone; local-dir → in place. */
export async function acquire(target) {
    const kind = await resolveInput(target);
    if (kind === 'local-dir') {
        return { kind, root: target, cleanup: async () => { } };
    }
    return cloneHostile(target);
}
async function cloneHostile(url) {
    ...
    try {
        ...
        await exec('git', ['-c', 'core.hooksPath=/dev/null', '-C', dir, 'checkout'], { env: { ...process.env, GIT_LFS_SKIP_SMUDGE: '1' } });
