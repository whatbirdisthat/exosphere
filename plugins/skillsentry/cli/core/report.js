...
export function renderMarkdown(report, target) {
    const lines = [];
 ...
 */
function appendExclusions(lines, report) {
 ...
 */
function appendDrift(lines, report) {
    ...
    if (drift.approvedHighCount > 0) {
        lines.push(`> NOTE: the lockfile approved ${drift.approvedHighCount} high-severity finding(s) — a lockfile cannot lower a fresh verdict (additive-only).`);
    }
...
export function renderJson(report, target) {
