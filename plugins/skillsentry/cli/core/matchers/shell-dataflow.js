...
const SHELL_KINDS = new Set(['script', 'hook']);
/**
...
const SENSITIVE_ENV = /\b(?:printenv|env)\s+\w*(?:SECRET|TOKEN|KEY|PASSWORD|CRED|EVIL)\w*|\$\{?\w*(?:SECRET|TOKEN|KEY|PASSWORD|CRED)\w*\}?/i;
/**
 ...
 */
export function rhsHasSource(rhs) {
...
const AUTORUN_LOCATION = /(?:~\/\.bashrc|~\/\.bash_profile|~\/\.profile|~\/\.zshrc|\/etc\/cron|crontab\b|\bsystemd\b|\.config\/systemd|authorized_keys|\.ssh\/authorized_keys)/;
/**
 ...
 */
export function sinkForLine(line, tainted) {
 ...
 */
export function forwardPass(file, seed = new Set()) {
    const matches = [];
 ...
 * variable reaches.
 */
export function shellTaintToSink(file) {
