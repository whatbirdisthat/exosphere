 ...
 */
export function diffCapabilities(freshFindings, freshHashes, lock) {
...
function stillPresent(cap, freshCaps) {
    const key = capabilitySetKey(cap);
    return freshCaps.some((c) => capabilitySetKey(c) === key);
}
function countChangedFiles(oldHashes, newHashes) {
    let changed = 0;
...
function invalidation(file, line, severity, approvedRule, approvedClass) {
    return {
