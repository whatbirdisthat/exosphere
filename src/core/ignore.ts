...
function compile(body: string): RegExp {
...
export function isExcluded(path: string, patterns: readonly IgnorePattern[]): boolean {
  let excluded = false;
...
export function applyIgnore(
  ...
  ignoreText: string,
): { kept: string[]; summary: ExclusionSummary } {
...
function blame(path: string, patterns: readonly IgnorePattern[]): string | undefined {
