 ...
 */
export function parseIncludes(content: string): ParsedInclude[] {
  ...
  for (const raw of content.split('\n')) {
    ...
    const rawPath = (m[1] ?? m[2] ?? m[3]) as string;
...
export function resolveInclude(
  fromPath: string,
  includePath: string,
): { readonly kind: 'in-tree'; readonly path: string } | { readonly kind: 'escape' } {
...
export function exportedTaint(file: FileRecord): Set<string> {
...
): { readonly imported: Set<string>; readonly escapes: RuleMatch[] } {
  const imported = new Set<string>();
  const escapes: RuleMatch[] = [];
 ...
 */
export function shellCrossfileTaintToSink(
  file: FileRecord,
  ...
  const byPath = new Map<string, FileRecord>(files.map((f) => [f.path, f]));
  const { imported, escapes } = gatherImports(file, byPath);

  const matches: RuleMatch[] = [...escapes];
...
function sourcedSiblingNames(file: FileRecord, byPath: ReadonlyMap<string, FileRecord>): string {
  const names: string[] = [];
  for (const inc of parseIncludes(file.content)) {
    const resolved = resolveInclude(file.path, inc.rawPath);
